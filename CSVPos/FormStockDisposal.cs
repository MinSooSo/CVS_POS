...
namespace CSVPos
{
    ...
    public partial class FormStockDisposal : Form
    {
        ...
        private void DisReturnPrice(int i)
        {
            con.Open();
            var stockpricecmd = new SqlCommand("si_DisReturnPrice", con);
            stockpricecmd.CommandType = CommandType.StoredProcedure;
            stockpricecmd.Parameters.AddWithValue("@DisReturn", i);
            var stockpricesdr = stockpricecmd.ExecuteReader();
            if (stockpricesdr.HasRows)
            {
                while (stockpricesdr.Read())
                {
                    allAbandonPrice += (int)stockpricesdr["금액"] * (int)stockpricesdr["갯수"];
                }
                if (i == 1)
                {
                    lblDisPrice.Text = allAbandonPrice.ToString() + " 원";
                }
                else
                {
                    lblreturnPrice.Text = allAbandonPrice.ToString() + " 원";
                }
            }
            allAbandonPrice = 0;
            con.Close();
